...
- **Honest calc (two modes via the toggle):** **"Spend it down"** depletes the pot to the country's **average age of death** (rounded life expectancy: EE 80, LV 77, LT 78), so the headline really does spend every euro by the time the average person passes. That shorter horizon (versus the earlier fixed age 90) is what lifts the monthly figure. It is safe only because retirement caps at **70**, below every Baltic life expectancy, so the horizon is always positive. **"Make it last"** is the perpetuity (draw only the real return, principal never touched): a lower but never-ending income. Each mode shows an honest counterpoint line (deplete: live past {age} and it runs dry; forever: smaller monthly, never runs out). "Years of freedom" in the stats strip is now death age minus retirement age.
